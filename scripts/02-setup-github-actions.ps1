...
Write-Host "AWS Credentials for GitHub Secrets:" -ForegroundColor Cyan
...
$awsRegion = Get-UserInput "Enter your AWS region" "ap-southeast-1"
...
try {
    ...
    if ($caller) {
        ...
        Write-Host "Could not verify AWS credentials. Please check your configuration." -ForegroundColor Yellow
    }
} catch {
    Write-Host "Could not test AWS credentials. Please verify manually." -ForegroundColor Yellow
}
...
try {
    ...
    Write-Host "GitHub CLI not found. You'll need to set secrets manually." -ForegroundColor Yellow
}
...
if ($ghAvailable) {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "GitHub CLI not authenticated. Please run 'gh auth login' first." -ForegroundColor Yellow
            $ghAvailable = $false
        }
    } catch {
        Write-Host "Could not set GitHub Secrets automatically." -ForegroundColor Yellow
        $ghAvailable = $false
    }
}
...
if (-not $ghAvailable) {
    Write-Host "Manual GitHub Secrets Setup:" -ForegroundColor Magenta
...
Write-Host "Updating .env file for GitHub Actions..." -ForegroundColor Cyan
...
Write-Host "GitHub Actions Workflow Information:" -ForegroundColor Magenta
Write-Host "=======================================" -ForegroundColor Magenta
...
Write-Host "Testing Your Workflow:" -ForegroundColor Magenta
Write-Host "=======================" -ForegroundColor Magenta
...
Write-Host "Monitoring and Verification:" -ForegroundColor Magenta
Write-Host "===============================" -ForegroundColor Magenta
...
Write-Host "Workshop Success Criteria:" -ForegroundColor Magenta
Write-Host "============================" -ForegroundColor Magenta
...
Write-Host "Next Step: Run 03-deploy-infrastructure.ps1" -ForegroundColor Cyan
